...
`ecs-physics` remains the deterministic 2D compatibility/reference solver used by the existing benchmark and invariant foundation. `ecs-physics-3d` is the true 3D solver used by the dedicated physics demo. Fixed geometry now uses swept AABB continuous collision detection across the full `(x, y, z, t)` step interval: exact integer-fraction TOI ordering chooses the earliest wall/floor/ceiling impact, deterministic Q32.32 subticks consume the remaining time after response, and the event loop fails closed if its bound is exhausted. Dynamic-vs-dynamic CCD remains the next horizon.
