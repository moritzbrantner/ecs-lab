...
`ecs-physics` remains the deterministic 2D compatibility/reference solver used by the existing benchmark and invariant foundation. `ecs-physics-3d` is the true 3D solver used by the dedicated physics demo. The 3D solver runs fixed and moving AABBs on one continuous `(x, y, z, t)` timeline: relative-motion swept AABB tests find the globally earliest exact integer-fraction TOI, every pair and tied X/Y/Z axis at that time is collected before response, and coupled normals are resolved through bounded deterministic passes. Q32.32 stays private intra-step state; final positions are rounded deterministically back to the integer ECS contract, and unsupported collision/contact churn fails closed.
