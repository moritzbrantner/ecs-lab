...
"""
if marker not in text or "struct RationalVector3" in text:
    raise SystemExit("unexpected response source before rational repair")
text = text.replace(marker, structs, 1)
...
text = replace_block(
    ...
    let normal_velocity_numerator =
        checked_dot(relative_velocity_numerator, geometry.normal)?;
    let normal_velocity_denominator = box_velocity.denominator;

""",
)
text = text.replace("if normal_velocity < 0\n", "if normal_velocity_numerator < 0\n", 1)
old_call = """        let impulse = normal_impulse(
            sphere_body,
            box_body,
...
"""
new_call = """        let impulse = normal_impulse(
            sphere_body,
            box_body,
            box_state.angular.orientation,
            box_offset,
...
if old_call not in text:
    raise SystemExit("normal impulse call shape changed unexpectedly")
...
text = replace_block(
    ...
    let local_impulse_numerator = rotate_inverse(state.angular.orientation, angular_impulse_numerator)?;
    let inertia = box_inertia(body)?;
...
text = replace_block(text, "fn add_linear_rotation(\n", "fn add_linear_impulse_axis(\n", "")
...
if test_marker not in text:
    raise SystemExit("test insertion point changed unexpectedly")
text = text.replace(test_marker, regression + test_marker, 1)

if "geometry.point)?" in text:
    raise SystemExit("rounded diagnostic point still feeds response math")
if "cross_i64_i128" in text or "normal_velocity < 0" in text:
    raise SystemExit("legacy rounded response path remains")
