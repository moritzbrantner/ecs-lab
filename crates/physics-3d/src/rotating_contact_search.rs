...
use crate::{
    ...
    RigidBoxFreeFlightConfig3d, RigidBoxFreeFlightError3d, RigidBoxWorldConfig3d,
    RigidBoxWorldError3d, RotationalSweepError3d, obb_contact_seed,
...
///
/// Every timing sample delegates to the canonical rigid-box free-flight primitive, so coarse sampling,
/// refinement, frontier reconstruction, and later frame replay share one gravity/position/orientation
/// rounding authority. The first coarse sample that reports an OBB contact establishes a clear/contact
...
///
/// End states are generated by the same canonical free-flight sampler used for bracket refinement, then
