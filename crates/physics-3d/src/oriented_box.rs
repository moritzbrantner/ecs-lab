...
use crate::{AngularError3d, BoxPlaneError3d, Orientation3d, oriented_box_vertices};
...
impl OrientedBox3d {
    #[must_use]
    pub const fn new(center: Position, half_extents: [i32; 3], orientation: Orientation3d) -> Self {
        Self {
            center,
            half_extents,
            orientation,
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidHalfExtents => write!(
                formatter,
                "oriented-box SAT requires strictly positive half extents"
            ),
            ...
            Self::ArithmeticOverflow => write!(formatter, "oriented-box SAT arithmetic overflowed"),
        }
    }
}

impl std::error::Error for OrientedBoxError3d {}
...
/// Evaluates exact integer SAT semantics for the quantized Rust-owned vertices of two oriented boxes.
///
/// The query tests the three face normals of each box plus all nine edge-cross-edge axes, removes exact
/// duplicate axes in stable feature order, and treats touching as contact. The returned support masks are
/// a bounded manifold seed: they identify the tied extreme vertices on the minimum-penetration axis but
/// deliberately do not yet perform polygon clipping or impulse response. That keeps this stage-3 slice
/// truthful while later manifold reduction and coupled angular response are added separately.
///
/// # Errors
///
/// Returns [`OrientedBoxError3d`] for invalid/degenerate oriented geometry or checked arithmetic overflow.
pub fn obb_contact_seed(
    left: OrientedBox3d,
    right: OrientedBox3d,
) -> Result<Option<ObbContactSeed3d>, OrientedBoxError3d> {
    let left_vertices = vertices(left)?;
    let right_vertices = vertices(right)?;
...
fn vertices(box_shape: OrientedBox3d) -> Result<[Position; 8], OrientedBoxError3d> {
...
fn map_geometry_error(error: BoxPlaneError3d) -> OrientedBoxError3d {
    match error {
        BoxPlaneError3d::InvalidHalfExtents => OrientedBoxError3d::InvalidHalfExtents,
        BoxPlaneError3d::Angular(error) => OrientedBoxError3d::Angular(error),
        ...
        | BoxPlaneError3d::FrictionOutOfRange(_) => OrientedBoxError3d::ArithmeticOverflow,
    }
}

fn box_edges(vertices: &[Position; 8]) -> Result<[[i128; 3]; 3], OrientedBoxError3d> {
    Ok([
...
fn position_delta(left: Position, right: Position) -> Result<[i128; 3], OrientedBoxError3d> {
    Ok([
        i128::from(right.x)
...
fn dot_position(position: Position, axis: [i128; 3]) -> Result<i128, OrientedBoxError3d> {
    checked_dot(
        [
...
mod tests {
    ...
    fn cube(center: Position) -> OrientedBox3d {
        OrientedBox3d::new(center, [1, 1, 1], Orientation3d::IDENTITY)
    }

    #[test]
    fn aligned_face_touch_has_stable_four_vertex_supports() {
        let contact =
            obb_contact_seed(cube(Position::new3(0, 0, 0)), cube(Position::new3(2, 0, 0)))
    ...
    fn aligned_edge_and_corner_touch_preserve_minimum_axis_ties() {
        let edge = obb_contact_seed(cube(Position::new3(0, 0, 0)), cube(Position::new3(2, 2, 0)))
            .expect("valid OBB pair")
            .expect("edge-touching cubes should contact");
        assert_eq!(edge.minimum_axis_ties, 2);

        let corner = obb_contact_seed(cube(Position::new3(0, 0, 0)), cube(Position::new3(2, 2, 2)))
            .expect("valid OBB pair")
    ...
    fn differently_rotated_boxes_use_non_aabb_sat_axes() {
        ...
        let left = OrientedBox3d::new(Position::new3(0, 0, 0), [4, 1, 1], left_orientation);
        let right = OrientedBox3d::new(Position::new3(0, 0, 0), [4, 1, 1], right_orientation);
    ...
    fn swapping_boxes_reorients_contact_axis() {
        let left = cube(Position::new3(0, 0, 0));
        let right = cube(Position::new3(2, 0, 0));
    ...
    fn invalid_dimensions_and_zero_orientation_fail_closed() {
        let invalid =
            OrientedBox3d::new(Position::new3(0, 0, 0), [1, 0, 1], Orientation3d::IDENTITY);
        assert_eq!(
            obb_contact_seed(invalid, cube(Position::new3(0, 0, 0))),
        ...
        assert!(matches!(
            obb_contact_seed(invalid_orientation, cube(Position::new3(0, 0, 0))),
